...
var fetch = function (callback) {
    ...
    xhr.open("GET", "https://cobratony.firebaseio.com//chirps/.json", true);
    xhr.onload = function () {
...
function showresultstopanel(data) {
    ...
    for (var i in data) {
        ...
        htmlstrtopanel += "<span class='chat-img pull-left'><img src='http://s2.quickmeme.com/img/3e/3e893cb199219590fc87a39d5bd6008ed5c8852291acde1ce1ceae1d20e76add.jpg' alt='User Avatar' class='img-circle' /></span>";
...
function sendToAjax(objToPost) {
    ...
    request.open('POST', 'https://cobratony.firebaseio.com//chirps/.json', true);
...
function dodeletetodo(index) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("DELETE", "https://cobratony.firebaseio.com//chirps/" + todoArray[index].id + "/.json", true)
    xhr.send();
    xhr.onload = function () {
        if (this.status >= 200 && this.status < 400) {
            fetch(showresults);
